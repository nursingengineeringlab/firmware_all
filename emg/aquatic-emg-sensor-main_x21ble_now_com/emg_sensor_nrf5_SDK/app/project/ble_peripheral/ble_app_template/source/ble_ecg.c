...
/* Private defines ---------------------------------------------------- */
#define BLE_UUID_ECG_RAW_DATA_CHARACTERISTIC          (0x4235)
#define BLE_UUID_ECG_HEART_RATE_CHARACTERISTIC        (0x4236)
#define BLE_UUID_ECG_RESPIRATION_RATE_CHARACTERISTIC  (0x4237)
...
static const uint16_t BLE_UUID_CHAR[] = {
  BLE_UUID_ECG_RAW_DATA_CHARACTERISTIC,
  BLE_UUID_ECG_HEART_RATE_CHARACTERISTIC,
  BLE_UUID_ECG_RESPIRATION_RATE_CHARACTERISTIC
};
...
uint32_t ble_ecg_init(ble_ecg_t *p_ecg, ble_ecg_init_t const *p_ecg_init)
{
  ...
  err_code = m_ble_ecg_add_char(p_ecg, p_ecg_init, BLE_ECG_RAW_DATA_CHAR);
  VERIFY_SUCCESS(err_code);

  err_code = m_ble_ecg_add_char(p_ecg, p_ecg_init, BLE_ECG_HEART_RATE_CHAR);
  VERIFY_SUCCESS(err_code);

  return m_ble_ecg_add_char(p_ecg, p_ecg_init, BLE_ECG_RESPIRATION_RATE_CHAR);
}

ret_code_t ble_ecg_update(ble_ecg_t *p_ecg, uint16_t acc, uint16_t conn_handle, ble_ecg_charaterictic_t charac)
{
  ret_code_t err_code;
  ...
  if (conn_handle != BLE_CONN_HANDLE_INVALID)
  {
    ...
    hvx_params.handle = p_ecg->ecg_char_handles[charac].value_handle;
...
static ret_code_t m_ble_ecg_add_char(ble_ecg_t *p_ecg, const ble_ecg_init_t *p_ecg_init, ble_ecg_charaterictic_t charac)
{
  ...
  return characteristic_add(p_ecg->service_handle, &add_char_params, &(p_ecg->ecg_char_handles[charac]));
}
